    ...
    <div className="flex flex-col">
      ...
      <div className="px-4 py-3">
        <motion.div
          ...
          className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-card to-card border border-primary/20 p-4"
          style={{ boxShadow: 'var(--app-gold-shadow)' }}
        >
          <div className="relative z-10">
            <p className="text-foreground font-bold text-sm mb-1">Ready to vibe? 🎧</p>
            <p className="text-muted-foreground text-xs mb-3">Book your event with DJ Soundzy today</p>
            <Button
              onClick={() => navigate('/book')}
              size="sm"
              className="h-9 bg-primary text-primary-foreground font-bold text-xs rounded-xl"
            >
              Book Now
            </Button>
          </div>
      ...
      <div className="px-4 pb-4">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-bold text-foreground">Community Feed</h2>
          <Button
            size="sm"
            variant="ghost"
            onClick={() => navigate('/create-post')}
            className="h-8 text-primary text-xs"
          >
  ...
  return (
    <div className="px-4 py-2">
      <div className="bg-card rounded-2xl border border-border p-3 flex items-center gap-3">
        {mixtape.cover_image && (
          <img src={mixtape.cover_image} alt={mixtape.title} className="h-14 w-14 rounded-xl object-cover" />
        )}
        <div className="flex-1 min-w-0">
          <p className="text-xs font-bold text-foreground truncate">{mixtape.title}</p>
          <p className="text-[10px] text-muted-foreground">{mixtape.artist_name}</p>
          {mixtape.genre && <Badge variant="outline" className="text-[9px] h-4 mt-1 border-secondary/30 text-secondary">{mixtape.genre}</Badge>}
        </div>
        <Button size="sm" className="h-8 w-8 rounded-full bg-primary text-primary-foreground p-0">
    ...
    <Card className="overflow-hidden border-border/50 bg-card">
      <CardContent className="p-4">
        ...
        <div className="flex items-center gap-2 mb-3">
          <div className="h-8 w-8 rounded-full bg-secondary/20 flex items-center justify-center text-secondary text-xs font-bold">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              {post.is_guest && (
                <Badge variant="outline" className="text-[9px] h-4 border-muted-foreground/30 text-muted-foreground">Guest</Badge>
              )}
            </div>
        ...
        <p className="text-sm text-foreground/90 leading-relaxed mb-3">
          ...
          {isLong && !expanded && (
            <button onClick={() => setExpanded(true)} className="text-primary text-xs ml-1">Read more</button>
          )}
        </p>
