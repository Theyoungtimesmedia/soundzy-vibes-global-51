...
import { Send, Sparkles, ArrowLeft, MessageCircle, Bot, X } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-60px-env(safe-area-inset-bottom,0px))]">
      ...
      <div className="px-4 py-3 border-b border-border flex items-center gap-3">
        {aiMode ? (
          <>
            <button onClick={() => setAiMode(false)} className="tap-target"><ArrowLeft className="h-5 w-5 text-muted-foreground" /></button>
            <div className="h-10 w-10 rounded-full bg-secondary/20 flex items-center justify-center">
              <Bot className="h-5 w-5 text-secondary" />
            </div>
            <div className="flex-1">
              <p className="text-sm font-bold text-secondary">Soundzy AI 🤖</p>
              <p className="text-[10px] text-muted-foreground">Ask about pricing, services & more</p>
            </div>
          </>
        ) : (
          <>
            ...
            <Button size="sm" variant="outline" onClick={() => setAiMode(true)} className="h-8 text-xs border-secondary/30 text-secondary rounded-xl gap-1">
          ...
          <>
            {aiMessages.length === 0 && (
              <div className="text-center py-8">
                ...
                <p className="text-sm font-bold mb-1">Soundzy AI</p>
                <p className="text-xs text-muted-foreground mb-4">Ask me about pricing, availability, services, or event planning!</p>
              </div>
            )}
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 ${
                  msg.role === 'user'
                    ...
                    : 'bg-secondary/10 text-foreground border border-secondary/20 rounded-bl-md'
                }`}>
                  {msg.role === 'assistant' && <p className="text-[10px] text-secondary font-bold mb-1">🤖 Soundzy AI</p>}
            ...
            {aiStreaming && aiMessages[aiMessages.length - 1]?.role !== 'assistant' && (
              <div className="flex justify-start">
                <div className="bg-secondary/10 border border-secondary/20 rounded-2xl rounded-bl-md px-4 py-3">
                  ...
                  <motion.div key={msg.id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 ${
                      isMe ? 'bg-primary text-primary-foreground rounded-br-md'
                        : msg.is_ai ? 'bg-secondary/20 text-foreground rounded-bl-md'
          ...
          <div className="flex gap-2">
            {quickChips.map((chip) => (
              <button key={chip.label} onClick={() => sendAiMessage(chip.msg)}
                className="shrink-0 px-3 py-1.5 rounded-full text-[10px] font-medium bg-secondary/10 border border-secondary/20 text-secondary hover:bg-secondary/20 transition-colors tap-target">
                {chip.label}
              </button>
            ))}
          </div>
      ...
      <div className="px-4 py-3 border-t border-border flex gap-2">
        ...
        <Button
          ...
          size="icon"
          className={`h-10 w-10 rounded-xl ${aiMode ? 'bg-secondary text-white' : 'bg-primary text-primary-foreground'}`}
        >
